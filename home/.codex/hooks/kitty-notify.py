...
# Notification types that do not mean Codex is waiting on you. `idle_prompt`
# is the one that matters: Codex sends it messageIdleNotifThresholdMs
...
def notify(title: str, body: str, session_id: str) -> None:
    subprocess.run([NOTIFY, "-i", f"codex-{session_id}", title, body], capture_output=True)
